...
- **A GGUF without an embedded chat template gets ChatML** — `llama-server` substitutes its ChatML fallback and reports it as the model's template, so the server cannot tell the two apart and chat mode is always accepted on `llama-cpp`. Check with `/template` if a model's output looks off. Only `transformers` models without a template are refused with HTTP 400.
